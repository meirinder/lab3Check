...
LevelsViewModel::LevelsViewModel(int a, int b, int c, int d, int m, int n, std::vector<QColor *> colors)
{
    this->a = a;
    this->b = b;
    this->c = c;
...
void LevelsViewModel::start(int width,int height)
{
    ...
    foreach (double var, val) {
        ...
        if(!check){
            divide.push_back(LevelsBoundaries(colInd.back(),var,var));
        }
    }
    colorss = divide;
}
...
double LevelsViewModel::aproximation(double x, double y, double k)
{
    return x+k*(y-x);
}

double LevelsViewModel::f(double x,double y){
    return x+y;
}
