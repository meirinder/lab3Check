...
WorkspaceViewModel::WorkspaceViewModel(int a, int b, int c, int d, int m, int n, std::vector<QColor *> colors)
{
    this->isIsolined = false;
    this->isGrided = false;
...
void WorkspaceViewModel::start(int width, int height)
{
    for(int u=0;u<width;u++){
        for(int v=0;v<height;v++){
            ...
            double z2=f(x0,y0);
            double z3=f(x1,y0);
            double z01=aproximation(z1,z0,(double)(x-x0)/(x1-x0));
            double z23=aproximation(z2,z3,(double)(x-x0)/(x1-x0));
            val.push_back(aproximation(z23,z01,(double)(y-y0)/(y1-y0)));
        }
    }
    ...
    foreach (double var, val) {
        ...
        if(!check){
            divide.push_back(WorkBoundaries(colInd.back(),var,var));
        }
    }
...
double WorkspaceViewModel::aproximation(double x, double y, double k)
{
    return x+k*(y-x);
}

double WorkspaceViewModel::f(double x,double y){
    return sin(x)+cos(y);
}
